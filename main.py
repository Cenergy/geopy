...
class Publisher:
    ...
    def __init__(self, url, user='admin', pwd='geoserver'):
        ...
        self.__data_type__ = {'.tif': 'GeoTIFF', '.img': 'ERDASImg'}
    ...
    def __ensure_workspace(self):
        ...
        ws_row_ = self.__row_of_table_item(('1', self.__ws__), path_,
                                           self.__ws__)
        if ws_row_ is not None:
            ...
            self.brower.find_element_by_xpath(
                '//*[@id="page"]/div[1]/div[2]/ul/li[1]/a').click()
            ...
            Publisher.__set_e_value__(
                self.brower.find_element_by_id('name'), self.__ws__)
            uri_ = self.__ws_uri_ + self.__ws__
            Publisher.__set_e_value__(
                self.brower.find_element_by_id('uri'), uri_)
            self.brower.find_element_by_id('default').click()

            self.brower.find_element_by_xpath(
                '//*[@id="page"]/div[2]/form/ul/li[4]/a').click()

            ws_row_ = self.__row_of_table_item(('1', self.__ws__), path_,
                                               self.__ws__)
            if ws_row_ is None:
                print(
                    'workspace %s create failed, please contact administrator.'
                    % self.__ws__)
                return False
        ...
        for i in range(1, 5):
            w_service = service_.find_element_by_xpath(
                './/li[' + str(i) + ']/input')
            if not w_service.is_selected():
                w_service.click()

        self.brower.find_element_by_xpath(
            '//*[@id="page"]/div[2]/form/ul/li[5]/a').click()
        return True
    ...
    def __add_datastore(self, dir_, file_):
        ...
        path_ = '//*[@id="page"]/div[2]/div[1]/table/tbody'
        if self.__row_of_table_item(('2', self.__ws__, '3', name_), path_,
                                    name_):
        ...
        fieldset_ = self.brower.find_element_by_xpath(
            '//*[@id="page"]/div[2]/form/ul/li[2]/fieldset')
        # workspace selected
        Select(fieldset_.find_element_by_xpath(
            './/div[1]/div/select')).select_by_visible_text(self.__ws__)
        # set datastore name
        Publisher.__set_e_value__(
            fieldset_.find_element_by_xpath('.//div[2]/div/input'), name)
        # set datastore description
        Publisher.__set_e_value__(
            fieldset_.find_element_by_xpath('div[3]/div/input'),
            'datastore created with %s automatically' % tif)

        # set datastore source tif file
        path_ = '//*[@id="page"]/div[2]/form/ul/li[3]/div/fieldset/div/span/div[1]/input'
        Publisher.__set_e_value__(
            self.brower.find_element_by_xpath(path_), 'file://' + tif)

        # 确定
        self.brower.find_element_by_xpath(
            '//*[@id="page"]/div[2]/form/div[4]/a[1]').click()
        return True
    ...
    def __publish_layer(self, name, isnew_data):
        ...
        if not isnew_data:
            ...
            Select(self.brower.find_element_by_xpath(
                path_)).select_by_visible_text(ws_name_)

        path_ = '//*[@id="page"]/div[2]/div[1]/div[2]/div/table/tbody'
        publish_row_ = self.__row_of_table_item(('3', '发布'), path_, '', 1800
                                                if isnew_data else 3)
        if publish_row_ is None:
            print(
                'problem may have occured in server, please restart web server.'
            )
            return False
        ...
        publisher.__set_e_value__(
            self.brower.find_element_by_xpath('//*[@id="declaredSRS"]/input'),
            self.__epsg__)
        # override parameters
        ...
        Publisher.__set_e_value__(
            over_field_.find_element_by_xpath('.//div[1]/span/span/input'),
            self.__opacity_color__)
        Publisher.__set_e_value__(
            over_field_.find_element_by_xpath('.//div[2]/span/div/input'),
            self.__tile_size__)
        ...
        self.brower.find_element_by_xpath(
            '//*[@id="page"]/div[2]/form/div[3]/a[1]').click()
        return True
    ...
    def __update_layer_group(self, name):
        ...
        path_ = '//*[@id="page"]/div[2]/div[1]/table/tbody'
        lg_row = self.__row_of_table_item(('1', self.__layer_group__), path_,
                                          self.__layer_group__)
        if lg_row is not None:
            ...
            while True:
                if self.__row_of_table_item(
                    ('3@span', self.__ws__ + ':' + name),
                        path_ + '/table/tbody'):
                    print('layer %s already existed in group %s' %
                          (name, self.__layer_group__))
                    return
                next_page_ = self.brower.find_element_by_xpath(
                    path_ + '/span/span[1]/a[3]')
            ...
            Publisher.__set_e_value__(
                self.brower.find_element_by_id('name'), self.__layer_group__)
            Publisher.__set_e_value__(
                self.brower.find_element_by_id('title'), self.__layer_group__)
            # select workspace.
            path_ = '//*[@id="page"]/div[2]/form/div[2]/div[2]/ul/li[4]/select'
            Select(self.brower.find_element_by_xpath(
                path_)).select_by_visible_text(self.__ws__)

        # add new layer to layer group.
        self.brower.find_element_by_xpath(
            '//*[@id="page"]/div[2]/form/div[2]/div[2]/ul/li[10]/div/fieldset/ul/li[1]/a'
        ).click()
    ...
    def __seed_gwc(self):
        ...
        self.brower.find_element_by_xpath(
            '//*[@id="seed"]/table/tbody/tr[8]/td[2]/input').click()
    ...
    def __row_of_table_item(self, cmps, tbody, filter_='', wait=2):
        ...
        cmp_pv = [(f_path(cmps[i - 1].split('@')), cmps[i])
                  for i in range(1, len(cmps), 2)]
...
def processArg(p):
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "hwulegsc",
            ["ws", "url", "layers", "epsg", "gridset", "size", "color"])
    except getopt.GetoptError:
        print(
            'test.py  -w <workspace> -u <workurl> -l <layergroup> -e <epsg> -g <gridset> -s <tilesize> -c <color>'
        )
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            ...
            p.set_workspace_uri(arg)
        elif opt in ("-l", "--layers"):
            p.set_layer_group(arg)
        elif opt in ("-e", "--epsg"):
            p.set_epsg(arg)
        elif opt in ("-g", "--gridset"):
            p.set_gridset(arg)
        elif opt in ("-s", "--size"):
            p.set_tile_size(arg)
        elif opt in ("-c", "--color"):
            p.set_opacity_color(arg)
